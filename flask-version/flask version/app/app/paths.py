...
def graph():
    ...
    if form.validate_on_submit():
        if form.days.data:
            ...
            if len(csv.columns) == 2:
                rfr_data, rfr_error_rate = Univariate_forecasting.forecast_algorithm("rfr", n, current_user.csv_file)
                svr_data, svr_error_rate = Univariate_forecasting.forecast_algorithm("svr", n, current_user.csv_file)
                dtr_data, dtr_error_rate = Univariate_forecasting.forecast_algorithm("dtr", n, current_user.csv_file)
                xgb_data, xgb_error_rate = Univariate_forecasting.forecast_algorithm("xgb", n, current_user.csv_file)
            elif len(csv.columns) > 2:
                rfr_data, rfr_error_rate = multivariate_forecasting.forecast_algorithm("rfr", n, current_user.csv_file)
                svr_data, svr_error_rate = multivariate_forecasting.forecast_algorithm("svr", n, current_user.csv_file)
                dtr_data, dtr_error_rate = multivariate_forecasting.forecast_algorithm("dtr", n, current_user.csv_file)
                xgb_data, xgb_error_rate = multivariate_forecasting.forecast_algorithm("xgb", n, current_user.csv_file)
    ...
    download_file(labels, rfr_data, svr_data, xgb_data, dtr_data)
...
def download():
    ...
    if form2.validate_on_submit():
        path = ROOT_DIR + "/app/static/downloads/data.csv"
